...
| A7 | **The burnt-zone temperature derivative is ~100x too large, and B15 does not explain it.** `dudTb` comes out around 172,000 J/(kg K) during expansion where the baseline trace's own `Tb` column implies about 1,660. It sits in the denominator of the burnt temperature equation, so `dTb/dtheta` lands 99 per cent short (-8 K/rad against the -862 K/rad a central difference of the trace requires) and the pressure equation 22 to 32 per cent short, leaving cylinder pressure about 0.7 per cent out through combustion and expansion. Compression is unaffected and matches to 0.08 per cent, because the unburnt branch of `ReturnProps` passes a zero `dRdT` (`GASPROPS.PAS:296`) and never touches the equilibrium temperature derivatives - which is what localises the fault to the path through `Eqbm1.dxdT`. **This casts doubt on B15.** That entry says the original inflates every equilibrium temperature derivative by `sqrt(101325)`, and it is reproduced verbatim and pinned by `EquilibriumSolverTests`. But `dudT` is built from `dxdT` (`GASPROPS.PAS:320-322`), so if the original really carried that inflation its own `Tb` would barely fall - and the trace shows it falling exactly as sound thermodynamics requires. Either `Partial_dxd` does not use pascals after all, or the port applies the scaling somewhere the original does not. Re-read `go2` and `Partial_dxd` against the port before going further; whichever way it resolves, B15's verdict and the test pinning it both have to change | Open, blocks closing phase 4a |
