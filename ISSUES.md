...
| A16 ([#129](https://github.com/pangtuwi/ESA.NET/issues/129)) | **A failed sweep row leaves no folder, and its manifest points at one that does not exist.** `MainWindowViewModel.ArchiveRow` writes the manifest's folder line, then returns before creating anything when `row.Result` is null — which `MultiRunner.RunRow` makes it exactly when a row threw (`MultiRunner.cs:120-131`), so the case where the operator most wants something to look at is the one that leaves nothing. No `RowNN_...` folder, no `inputs/`, no trace, and a `run.txt` naming a folder that was never created. `SimulDat.txt` gains no row either, so its lines stop lining up with the grid's: a five-row sweep whose third row fails writes rows 1, 2, 4 and 5 with nothing marking the gap, which compounds C6 and C14. It contradicts a decision taken deliberately a few lines earlier — `ArchiveRun` keeps the folder for a failed single-point run and writes the manifest anyway, its comment saying "the manifest saying why is the part worth keeping". **Fix**: write the row folder regardless and put the failure in it, and settle whether `SimulDat.txt` should carry a row number or a placeholder line so the file is self-describing. Found while auditing the run-folder work of [`f95642e`](https://github.com/pangtuwi/ESA.NET/commit/f95642e4bb77e56004cfe6f339f327dc3c37bae2) | Open |
...
The port now has a **data folder**, added in
[`f95642e`](https://github.com/pangtuwi/ESA.NET/commit/f95642e4bb77e56004cfe6f339f327dc3c37bae2): `Documents/ESA`, holding `Engines` and `Runs`,
