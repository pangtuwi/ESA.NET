...
| A8 | **Cylinder pressure runs up to 0.72 per cent low over the first eight steps after the spark**, decaying monotonically to zero thereafter; the rest of combustion and all of expansion sit inside 0.25 per cent. The shape says starting value, not wrong equation: the burnt zone is seeded at the spark by `InitialTb`'s isenthalpic iteration, and there the burnt mass fraction is at its 0.01 clamp, where the temperature equation is stiffest. Pinned at its measured size by `CycleSolverTests` so a regression shows up | Open, small |
